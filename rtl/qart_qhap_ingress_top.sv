module qart_qhap_ingress_top #(parameter integer WATCHDOG_CYCLES=32)(
 ...
 logic[2:0] beat_count; logic crc_start,crc_feed,frame_pending,rx_valid_latched,rx_fault_latched; logic[5:0] crc_wait; logic[31:0]crc_calc,crc_word_latched;
 ...
 always_ff @(posedge clk) begin
  if(!rst_n) begin beat_count<=0;crc_start<=0;crc_feed<=0;frame_pending<=0;rx_valid_latched<=0;rx_fault_latched<=0;crc_wait<=0;crc_word_latched<=0; end
  else begin
   crc_start<=0; crc_feed<=0;
   if(frame_pending && crc_wait==0) begin frame_pending<=0; rx_valid_latched<=0; rx_fault_latched<=0; end
   if(frame_pending && crc_wait!=0) crc_wait<=crc_wait-1'b1;
   if(s_axis_tvalid&&s_axis_tready) begin
    if(beat_count==0) crc_start<=1;
    if(beat_count<7) crc_feed<=1;
    if(beat_count==7 && s_axis_tlast) begin crc_word_latched<=s_axis_tdata; frame_pending<=1; crc_wait<=6'd6; rx_valid_latched<=1; rx_fault_latched<=0; end
