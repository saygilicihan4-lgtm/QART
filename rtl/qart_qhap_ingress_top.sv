module qart_qhap_ingress_top #(parameter integer WATCHDOG_CYCLES=32)(
 ...
 logic[2:0] beat_count; logic crc_start,crc_feed,frame_pending; logic[31:0]crc_calc,crc_word_latched;
 ...
 always_ff @(posedge clk) begin
  if(!rst_n) begin beat_count<=0;crc_start<=0;crc_feed<=0;frame_pending<=0;crc_word_latched<=0; end
  else begin
   crc_start<=0; crc_feed<=0;
   if(frame_pending) frame_pending<=0;
