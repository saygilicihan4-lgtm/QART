module qart_qhap_ingress_top #(parameter integer WATCHDOG_CYCLES=32)(
 ...
 logic[2:0] beat_count; logic crc_start,crc_feed; logic[31:0]crc_calc;
 ...
 always_ff @(posedge clk) begin
  if(!rst_n) begin beat_count<=0;crc_start<=0;crc_feed<=0;end else begin
 ...
 qart_fail_closed_ingress #(.WATCHDOG_CYCLES(WATCHDOG_CYCLES)) gate(.clk(clk),.rst_n(rst_n),.arm(arm),.clear_fault(clear_fault),.resync(resync),.resync_value(resync_value),.frame_valid(fv),.protocol_ok(!ff),.crc_ok(crc_received==crc_calc),.magic(magic),.version(version),.kind(kind),.seq(seq),.channel(channel),.action(action),.amplitude(amplitude),.duration(duration),.permit(permit),.fault(fault),.safe_noop(safe_noop),.expected_seq(expected_seq));
endmodule
