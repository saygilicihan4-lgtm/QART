module qart_qhap_ingress_top #(parameter integer WATCHDOG_CYCLES=32)(
 ...
 logic[2:0] beat_count; logic crc_start,crc_feed,frame_pending,rx_valid_latched,rx_fault_latched,crc_ready,crc_done_latched; logic[31:0]crc_calc,crc_word_latched,crc_data_latched; logic[2:0] crc_index_latched;
 ...
 always_ff @(posedge clk) begin
  if(!rst_n) begin beat_count<=0;crc_start<=0;crc_feed<=0;frame_pending<=0;rx_valid_latched<=0;rx_fault_latched<=0;crc_done_latched<=0;crc_word_latched<=0;crc_data_latched<=0;crc_index_latched<=0; end
  else begin
   crc_start<=0; crc_feed<=0;
   if(crc_ready) crc_done_latched<=1;
   if(frame_pending && crc_done_latched) begin frame_pending<=0; rx_valid_latched<=0; rx_fault_latched<=0; crc_done_latched<=0; end
   if(s_axis_tvalid&&s_axis_tready) begin
    if(beat_count==0) begin crc_start<=1; crc_done_latched<=0; end
    if(beat_count<7) begin crc_feed<=1; crc_data_latched<=s_axis_tdata; crc_index_latched<=beat_count; end
 ...
 qart_qhap_crc_stream cs(.clk(clk),.rst_n(rst_n),.start(crc_start),.tdata(crc_data_latched),.beat_valid(crc_feed),.beat_index(crc_index_latched),.crc(crc_calc),.ready(crc_ready));
 qart_fail_closed_ingress #(.WATCHDOG_CYCLES(WATCHDOG_CYCLES)) gate(.clk(clk),.rst_n(rst_n),.arm(arm),.clear_fault(clear_fault),.resync(resync),.resync_value(resync_value),.frame_valid(frame_pending && crc_done_latched),.protocol_ok(rx_valid_latched && !rx_fault_latched),.crc_ok(crc_word_latched==crc_calc),.magic(magic),.version(version),.kind(kind),.seq(seq),.channel(channel),.action(action),.amplitude(amplitude),.duration(duration),.permit(permit),.fault(fault),.safe_noop(safe_noop),.expected_seq(expected_seq));
endmodule
